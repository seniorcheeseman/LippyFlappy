...
async function powerOfLippyFlappy() {
    ...
    for (let i = 0; i < framesForVideo; i += 1) {
        ...
        if (mouth === '0') {
            ffmpeg.FS('writeFile', `tmp.${num}.png`, closedMouth);
        }
        else if (mouth === '50') {
            ffmpeg.FS('writeFile', `tmp.${num}.png`, halfOpenMouth);
        }
        else {
            ffmpeg.FS('writeFile', `tmp.${num}.png`, fullMouth);
        }
    }
    message.innerHTML = 'Start transcoding';
    await ffmpeg.run('-framerate', fps.toString(), '-pattern_type', 'glob', '-i', '*.png', '-i', audioFileName, '-c:a', 'copy', '-shortest', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', 'out.mp4');
...
function getAudioLevelOutputPerFrame(inputFilePath, target, PCM, videoLength) {
    console.log('InputFilePath: ' + inputFilePath);
    // Make sure we have files to use
    if (!target.files.length) {
        //TODO add Text that file not Selected
        return;
    }
    ...
    var fps = 60.0
        , sample_size = 48000.0
        , sample_rate = 2.0
    var framesForOneFrame = Math.floor(1 / fps * sample_size * sample_rate) // choose power of 2, 2048
    ...
    for (var i = 0; i < audioToFrames; i++) {
        ...
        var end = Math.min(audioFrameIdx + framesForOneFrame, PCM.length)
        var total = 0
        while (audioFrameIdx < end) {
            total += Math.abs(PCM[audioFrameIdx++])
        }
        rms = Math.sqrt(total / Math.max(end - start, 1))
        check = rms
        if (check < bottomLevel) {
            resultList.push(0)
        }
        else if (bottomLevel <= check < midlevel) {
            resultList.push(50)
        }
